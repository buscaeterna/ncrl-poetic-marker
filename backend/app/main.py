...
from .schemas import (CapabilitiesResponse, ErrorResponse, HealthResponse, JobCreate,
...
async def limit_body(request: Request, call_next):
    if request.method in {"POST", "PUT", "PATCH"}:
        ...
        if length and int(length) > settings.max_workspace_bytes:
            return Response(json.dumps({"code": "payload_too_large", "message": "Request exceeds 20 MiB limit"}), 413, media_type="application/json")
...
@app.get("/api/v1/ready", response_model=HealthResponse, responses={503: {"model": ErrorResponse}}, tags=["system"])
def ready(db: Session = Depends(database)) -> HealthResponse:
    try:
        db.execute(text("SELECT 1"))
    except Exception as exc:
        raise HTTPException(503, detail={"code": "database_unavailable", "message": str(exc)}) from exc
    return HealthResponse(service=settings.service_name, version=settings.version)
...
@app.post("/api/v1/projects", response_model=ProjectDetail, status_code=201, tags=["projects"])
...
@app.put("/api/v1/projects/{project_id}", response_model=ProjectDetail, responses={409: {"model": ErrorResponse}}, tags=["projects"])
...
@app.post("/api/v1/projects/{project_id}/jobs", response_model=JobResponse, status_code=201, tags=["jobs"])
...
@app.get("/api/v1/projects/{project_id}/jobs", response_model=list[JobResponse], tags=["jobs"])
...
@app.post("/api/v1/jobs/{job_id}/cancel", response_model=JobResponse, responses={409: {"model": ErrorResponse}}, tags=["jobs"])
